...
void UpdateProfiler(mj::Simulate* sim, const mjModel* m, const mjData* d)
{
    ...
    float sdata[6] = {static_cast<float>(m->nv),
...
void MakePhysicsSection(mj::Simulate* sim)
{
    ...
    for (int i = 0; i < sim->ui0.sect[SECT_PHYSICS].nitem; i++)
    {
        ...
        {
            if (mju::strcmp_arr(it->name, "Actuator Group Enable") && mju::strcmp_arr(it->name, "Contact Override")
    ...
    for (int i = 0; i < sim->jnt_type_.size() && itemcnt < mjMAXUIITEM; i++)
    {
        if ((sim->jnt_type_[i] == mjJNT_HINGE || sim->jnt_type_[i] == mjJNT_SLIDE))
        {
            ...
            if (sim->jnt_range_[i].has_value())
                mju::sprintf_arr(
                    defSlider[0].other, "%.4g %.4g", sim->jnt_range_[i]->first, sim->jnt_range_[i]->second);
...
void MakeControlSection(mj::Simulate* sim)
{
    mjuiDef defControl[] = {{mjITEM_SECTION, "Control", mjPRESERVE, nullptr, "AC"},
                            ...
                            // 添加新按钮
                            {mjITEM_CHECKBYTE, "Single Step", 2, &sim->single_step_mode, ""},  // 新增的按钮
                            {mjITEM_SEPARATOR, "Controls", 1},                                 // 添加分隔符
                            {mjITEM_END}};
    mjuiDef defSlider[] = {{mjITEM_SLIDERNUM, "", 2, nullptr, "0 1"}, {mjITEM_END}};
    ...
    for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM; i++)
    {
        ...
        if (sim->actuator_ctrlrange_[i].has_value())
            mju::sprintf_arr(defSlider[0].other,
                             "%.4g %.4g",
...
void UiEvent(mjuiState* state)
{
    ...
    if (state->type == mjEVENT_MOVE && state->dragrect == 3 && (sim->m_ || sim->is_passive_))
    {
        ...
        if (sim->pert.active)
        {
            ...
            {
                mjv_movePerturb(
                    sim->m_, sim->d_, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->pert);
            }
            else
            {
                mjv_movePerturbFromState(
                    &sim->scnstate_, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->pert);
            }
        }
        else
        {
            ...
            {
                mjv_moveCameraFromState(
                    &sim->scnstate_, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->cam);
            }
        }
        return;
    }
...
#undef X
    ...
    else
    {
        ...
        if (user_scn)
        {
            ...
            {
                std::memcpy(
                    scnstate_.scratch.geoms + scnstate_.scratch.ngeom, user_scn->geoms, sizeof(mjvGeom) * ngeom);
