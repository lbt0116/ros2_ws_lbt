...
MujocoInterface::MujocoInterface(std::shared_ptr<PinocchioInterface> pinocchio_interface)
    : Node("interface", rclcpp::NodeOptions().use_intra_process_comms(true)), pinocchioInterface_(std::move(pinocchio_interface))
{
    ...
    // 注册同步回调函数
    synchronizer_->registerCallback(std::bind(&MujocoInterface::sub_sensor_callback, this, std::placeholders::_1, std::placeholders::_2));
...
void MujocoInterface::sub_mujoco_callback(const custom_msgs::msg::MujocoMsg::SharedPtr msg)
{
    ...
    RCLCPP_INFO(this->get_logger(), "pub contact %.2d %.2d %.2d %.2d", msg->contact_state[0], msg->contact_state[1], msg->contact_state[2], msg->contact_state[3]);
}
