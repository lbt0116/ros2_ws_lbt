...
void PinocchioInterface::PinocchioInterfaceImpl::update_pinocchio(PinocchioInterface* pin_)
{
    ...
    pin_->baseSpatialInertiaMatrix << model.inertias[0].inertia().matrix(), Eigen::Matrix3d::Zero(), Eigen::Matrix3d::Zero(),
    ...
    for (int leg = 0; leg < 4; ++leg)
    {
        ...
        // 足端相对于髋关节在机身坐标系中的位置
        Eigen::Vector3d foot_pos_rel_hip_in_body = base_pose_in_world.actInv(hip_pose_in_world).rotation() * foot_pos_rel_hip_in_world;
        ...
        // 足端相对于髋关节的速度（机身坐标系）
        Eigen::Vector3d foot_vel_rel_hip_in_body = base_pose_in_world.actInv(hip_pose_in_world).rotation() * foot_vel_rel_hip_in_world;
...
void PinocchioInterface::set_mujoco_msg(const custom_msgs::msg::MujocoMsg::ConstSharedPtr& msg)
{
    ...
    // legForceInWorld = stdVectorToEigen(msg->ground_reaction_force, 3, cols);
}
