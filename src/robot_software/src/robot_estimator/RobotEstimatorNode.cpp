...
RobotEstimatorNode::RobotEstimatorNode(std::shared_ptr<PinocchioInterface> pinocchio_interface)
    : Node("estimate_node", rclcpp::NodeOptions().use_intra_process_comms(true)), pinocchioInterface_(std::move(pinocchio_interface)), eskf_(std::make_shared<EskfOnSe3>(std::move(pinocchio_interface)))
{
