...
namespace Galileo
{
    ...
    void EskfOnSe3::setTransMatrix()
    {
        ...
        C_mat.topLeftCorner<3, 3>() = Sophus::SO3d::hat(pin_->baseInertiaMatrix * w);
        C_mat.topRightCorner<3, 3>() = pin_->totalMass * Sophus::SO3d::hat(v);
        C_mat.bottomLeftCorner<3, 3>() = pin_->totalMass * Sophus::SO3d::hat(v);
        C_mat.bottomRightCorner<3, 3>() = Eigen::Matrix3d::Zero();

        const auto invJb = pin_->baseSpatialInertiaMatrix.inverse();
        ...
        Fn_mat.block<6, 6>(6, 0) = pin_->baseSpatialInertiaMatrix.inverse();
    }

    void EskfOnSe3::setMeasureMatrix()
    {
        H_mat.block(0, 0, 6, 6).setIdentity();
        H_mat.block(6, 15, 3, 3) = Eigen::Matrix3d::Identity() / pin_->totalMass;
    ...
    void EskfOnSe3::setErrorMatrix()
    {
        ...
        const Sophus::SE3d gm(Rm_, pm);

        Error_Vec.block(0, 0, 3, 1) = (g.inverse() * gm).log().tail<3>();
        Error_Vec.block(3, 0, 3, 1) = (g.inverse() * gm).log().head<3>();
        Error_Vec.block(6, 0, 3, 1) = am_ - (uc_ + Fe) / pin_->totalMass - grav;
        Error_Vec.block(9, 0, 3, 1) = wm_ - w;
        Error_Vec.block(12, 0, 3, 1) = vm_ - v;

        Obs_Vec << UtilFnc::invCayley2se3(gm.matrix()), wm_, vm_;
    ...
    {
        vm_ = -pin_->legVeloInWorld * phase.cast<double>() / phase.sum();
        uc_ = pin_->legForceInWorld * phase.cast<double>();

        am_ = pin_->baseAcc;
        wm_ = pin_->baseAngVelo;
        Rm_ = pin_->baseRotationMatrix;
